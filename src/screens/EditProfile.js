...
export default function EditProfile() {
  ...
  const onSave = async () => {
    ...
    if (navigation.canGoBack()) {
      // Normal edit flow — EditProfile was pushed onto the AppStack from Profile
      navigation.goBack();
    } else {
      // Onboarding flow — EditProfile is the only screen in EditProfileStackNavigator.
      // There's nothing to navigate to; force App to re-evaluate routeKey.
      await supabase.auth.refreshSession(); // fires onAuthStateChange → refreshRoute()
    }
    setSaving(false);
  };
