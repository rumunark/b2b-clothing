...
export default function ApproveRental() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Prevent self-rental
      if (user.id === buyerId) {
        Alert.alert('Error', 'You cannot approve a rental to yourself.');
        setProcessing(false);
        return;
      }
      ...
      // TODO: Create payment hold via edge function
      // const { data: payment } = await supabase.functions.invoke('create-payment-hold', {
      //   body: { request_id: requestId, amount: totalPrice }
      // });

      // Encrypt initial message
      const sellerPrivateKey = await getPrivateKey(user.id);
      const buyerPublicKey = await getPublicKey(buyerId);
      if (!sellerPrivateKey || !buyerPublicKey) throw new Error('Encryption keys not available');

      const initialMessage = {
        content: `Rental approved!\nStart: ${startDate}\nNights: ${nights}\n\nArrange a pickup time through chat.`,
        sender_id: user.id,
        created_at: new Date().toISOString(),
        read_at: null,
      };

      const encrypted = encryptMessage(JSON.stringify(initialMessage), buyerPublicKey, sellerPrivateKey);
      if (!encrypted) throw new Error('Message encryption failed');

      // Create chat linked to request
